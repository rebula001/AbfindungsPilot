...
> **In 30 Sekunden:** `npm run build` ist die Single-Source-of-Truth für „alles ist grün“ - sie startet Lint, Format, i18n-Check, TypeScript- und Vite-Build hintereinander. Für schnelles lokales Probieren gibt es `build:fast`. SonarQube läuft via PowerShell-Skript.

## NPM-Befehle

| Befehl | Zweck | CI-blockierend |
| --- | --- | --- |
| `npm run dev` | Vite Dev-Server, HMR auf `<http://localhost:5173>` | - |
| `npm run build` | `#strict#`: `check:all` + `vue-tsc -b` + `vite build` + `dist/` | ✅ |
...
| `npm run check:all` | Aggregat: `lint:check` + `format:check` + `i18n:check` (kein Build) | ✅ |
| `npm run lint:check` | ESLint (read-only) | ✅ |
| `npm run lint:fix` | ESLint + auto-fix | - |
| `npm run format:check` | Prettier (read-only) | ✅ |
| `npm run format:fix` | Prettier + auto-fix | - |
| `npm run i18n:check` | DE/ZH Key-Diff (siehe [05](./05-i18n.md)) | ✅ |
| `npm run sonar:all` | SonarQube Scan + Report fetchen | empfohlen vor Release |

**Faustregel:** lokal `dev` und `build:fast`, vor jedem Commit `build`.
...
Connected-Mode optional (Server: privater SonarQube oder SonarCloud). Konfig-Dateien:

| Datei | Inhalt | In Git? |
| --- | --- | --- |
| [`sonar-project.properties`](../../../frontend/sonar-project.properties) | URL, Token, Project-Key, Excludes | ❌ (enthält Tokens, in `.gitignore`) |
| [`scripts/sonar-scan.ps1`](../../../frontend/scripts/sonar-scan.ps1) | One-shot Scan + Report-Fetch | ✅ |
| [`.vscode/settings.json`](../../../.vscode/settings.json) | SonarLint Verbindung + Vue-False-Positives off | ✅ |

Wichtige Excludes:

```properties
sonar.exclusions=**/node_modules/**,**/dist/**,**/coverage/**,**/*.config.js,**/*.config.ts
sonar.cp.exclusions=src/i18n/**       # de.ts/zh.ts strukturidentisch (Design)
sonar.scm.disabled=false              # seit Git-Init aktiv (per-line blame)
```

Token-Trennung:

- `sonar.token` (`sqp_`) -> Scanner-Upload (`sonar-scanner.exe`)
- `sonar.report.token` (`squ_`) -> Web-API für Report-Fetcher
...
- User-Settings -> `sonarlint.connectedMode.connections.sonarqube` mit `serverUrl: '<your-sonarqube-url>'` (kein expliziter `connectionId` -> wird zu `<default>`)

Kontrolle: VS Code -> Output -> "SonarLint" Channel -> Log enthält `Bound to project 'Abfindungspilot'`.

### Vue 3 vs. SonarLint React-Hooks-Regel

In `.vscode/settings.json` deaktiviert:
...
Grund: SonarLint-Regel S6440 erkennt React-Hook-Aufrufe (`useXxx`) in Bedingungen / Schleifen - das ist für Vue 3 Composition API ein Falsch-Positiv.

## Git

| Datei | Zweck |
| --- | --- |
| [`/.gitignore`](../../../.gitignore) | Repo-Root, deckt alle Pfade ab (inkl. `frontend/sonar-project.properties`) |
