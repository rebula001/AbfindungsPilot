...
如果以后 App 不再有意使用 2026 年数值，而是要真正支持 2027 和 2028 年的计算，至少需要检查和更新下面这些内容。主要入口是 `frontend/src/calculation/constants.ts`、`frontend/src/i18n/de.ts`、`frontend/src/i18n/zh.ts`、`frontend/src/views/InputView.vue`、`frontend/src/views/CalculationView.vue`，以及 `.dev-docs/de` / `.dev-docs/zh` 下的计算文档。
...
重要：如果只是改数字，但常量名仍然叫 `_2026`，代码会越来越难维护。真正支持 2027/2028 时，要么明确继续保留“固定 2026 模型”，要么把常量改成按年份参数化的结构。
